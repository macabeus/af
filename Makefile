...
# a copy of the built ELF with a DWARF types-sidecar merged in (names from .symtab,
# declaration shapes from the sidecar). Consumed via decomp.yaml tools.asmlift.elf.
# Needs Docker (dockerized big-endian mips-linux-gnu-gcc); see tools/asmlift-sidecar.sh.
#
# The sidecar TU is GENERATED from the header list below (a build artifact, like the
# rest of build/). The list is CURATED, not a glob: the per-segment hex-named headers
# carry conflicting redeclarations (e.g. func_800FEBE0_jp in lbrtc.h vs 722880.h) and
# cannot all coexist in one TU. Add new headers here as they gain globals.
ASMLIFT_CTX_HEADERS := global.h \
	m_lib.h \
	m_common_data.h \
	m_field_info.h \
	overlays/gamestates/ovl_play/m_play.h \
	gfx.h \
	game.h \
	overlays/actors/player_actor/m_player.h \
	m_time.h \
	m_rcp.h \
	m_player_lib.h \
	audio.h \
...
build/asmlift-ctx.c:
	@mkdir -p build
	@printf '/* generated by make — the asmlift DWARF types-sidecar TU; see Makefile */\n' > $@
	@for h in $(ASMLIFT_CTX_HEADERS); do echo "#include \"$$h\""; done >> $@

.PHONY: asmlift-elf
asmlift-elf: build/asmlift-ctx.c
